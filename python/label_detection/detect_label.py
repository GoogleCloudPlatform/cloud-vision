...
# [START identify_label]
def identify_label(gcs_uri, max_results=4):
    """Uses the Vision API to identify label in the given image.

    Args:
        gcs_uri: A uri of the form: gs://bucket/object

    Returns:
        An array of dicts with information about label in the picture.
    """
    batch_request = [{
        'image': {
            'source': {
...
def main(gcs_uri):
    ...
    annotations = identify_label(gcs_uri)
    if not annotations:
        print('No label identified')
    else:
